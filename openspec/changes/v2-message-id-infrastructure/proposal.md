...
- **`lifecycle/types.py`**: New `DeliveryMode` enum (`STEER`, `QUEUE`) aligned with ACP v2 and OpenCode wire formats.
- **`orchestrator/session_pool.py`**: New `send_message()`, `run_agent()`, `revoke_message()`, `wait_for_completion()` public API methods. `receive_request()` deprecated, delegates to `send_message()`.
- **`capabilities/delegation.py`**: `DelegationService` protocol deprecated, delegates to `SessionPool.run_agent()`.
- **`capabilities/runloop_delegation.py`**: `RunLoopDelegationService` deprecated, delegates to `SessionPool.run_agent()`.
- **`capabilities/subagent_capability.py`**: Migrates from `ctx.delegation.spawn_subagent()` to `ctx.host.session_pool.run_agent()`.
- **Tests**: New tests for revoke/replace semantics, message_id propagation, ID-tracked feedback queue, DeliveryMode enum, SessionPool public API, and deprecation warnings.
