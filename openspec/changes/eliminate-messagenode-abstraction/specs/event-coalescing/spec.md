...
The EventBus SHALL accept `pydantic_ai.AgentStreamEvent` union types (`PartStartEvent`, `PartDeltaEvent`, `PartEndEvent`, `FunctionToolCallEvent`, `FunctionToolResultEvent`) as valid event payloads. The `drain_and_merge()` function SHALL coalesce native `PartDeltaEvent` instances using the same text/thinking/tool-call delta merging logic previously applied to custom `PartDeltaEvent` types.
