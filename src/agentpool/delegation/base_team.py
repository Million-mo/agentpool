"""Concrete team class with parallel/sequential execution modes.

This module consolidates the former ``Team`` (parallel) and ``TeamRun``
(sequential) classes into a single concrete :class:`BaseTeam` class that
dispatches to the appropriate graph execution path based on ``mode``.
"""
...
from agentpool.delegation.graph_sequential import (
    _SequentialGraphState,
...
TeamMode = Literal["parallel", "sequential"]
"""Execution mode for teams."""
...
class ExtendedTeamTalk(TeamTalk):
    """TeamTalk that also provides team run stats interface."""
...
class BaseTeam[TDeps, TResult](MessageNode[TDeps, TResult]):
    """Concrete team class supporting parallel and sequential execution.

    The ``mode`` parameter determines execution strategy:
    - ``"parallel"``: All members run concurrently via Fork + Join graph.
    - ``"sequential"``: Members run in order, chained via linear graph edges.
    """
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            agents: Team member nodes.
            mode: Execution mode - ``"parallel"`` or ``"sequential"``.
            name: Team name.
            description: Optional description.
            display_name: Optional display name.
            shared_prompt: Optional prompt prepended to all member inputs.
            mcp_servers: MCP servers for the team.
            event_configs: Event configurations.
            agent_pool: Pool the team belongs to.
            member_prompt_templates: Per-member prompt template configs.
            member_timeout: Maximum seconds each member may run.
            validator: Optional validator node (sequential mode only).
        """
    ...
    def __repr__(self) -> str:
        ...
        return f"{self.__class__.__name__}[{self.mode}][{len(self.nodes)}]{name}: {members}"
    ...
    def __or__(
        ...
        # If we're already sequential, extend it
        if self.mode == "sequential":
            if self.validator:
                # If we have a validator, create new sequential team to preserve validation
    ...
    async def execute(
        ...
        **kwargs: Any,
        ...
        """Execute the team, dispatching to parallel or sequential graph.
    ...
    async def execute_iter(
        ...
        *prompts: PromptCompatible | None,
        **kwargs: Any,
    ) -> AsyncIterator[Talk[Any] | Any]:
        """Yield results from execution, dispatching by mode.

        For sequential mode, yields AgentResponse and Talk objects in order.
        For parallel mode, yields AgentResponse objects as they complete.
        """
        if self.mode == "sequential":
            async for item in self._execute_sequential_iter(*prompts, **kwargs):
                yield item
        else:
    ...
    async def _execute_sequential(
        ...
        **kwargs: Any,
        ...
        """Run agents sequentially via pydantic-graph chain."""
        ...
        state = _SequentialGraphState(
            prompts=tuple(prompts_),
            kwargs=kwargs,
            connections=connections,
        )

        result = await run_sequential_graph(
            list(self.nodes),
            state,
    ...
    async def run(
        ...
        **kwargs: Any,
        ...
        """Run the team and return a combined message.

        For parallel mode, content is a list of all member contents.
        For sequential mode, content is the last member's content.
        """
    ...
    async def run_stream(
        ...
        **kwargs: Any,
        ...
        For parallel mode, all members stream concurrently.
        For sequential mode, members stream one after another.
        """
        if self.mode == "parallel":
    ...
    async def run_iter(
        ...
        **kwargs: Any,
        ...
        """Yield messages as they arrive from execution."""
        if self.mode == "parallel":
    ...
    async def _execute_sequential_iter(  # noqa: PLR0915
        self,
        *prompts: PromptCompatible,
        **kwargs: Any,
    ) -> AsyncIterator[Talk[Any] | Any]:
        """Yield results from sequential execution."""
        from itertools import pairwise

        from pydantic_graph import GraphBuilder, Step
        from pydantic_graph.id_types import NodeID

        from agentpool.delegation.graph_sequential import _SequentialGraphState
        from agentpool.talk.talk import Talk
        ...
        for i, node in enumerate(all_nodes):
            ...
            async def _step(ctx: Any, _node=node, _i=i) -> Any:
                ...
                start = perf_counter()
                if _i == 0:
                    result = await _node.run(*state.prompts, **state.kwargs)
                else:
                    result = await _node.run_message(ctx.inputs)
                timing = perf_counter() - start
                from agentpool.messaging import AgentResponse

                response = AgentResponse(agent_name=_node.name, message=result, timing=timing)
                state.responses.append(response)
                if _i < len(state.connections):
                    talk = state.connections[_i]
                    if result:
    ...
    # ── Shared helpers (from former Team class) ──────────────────────
